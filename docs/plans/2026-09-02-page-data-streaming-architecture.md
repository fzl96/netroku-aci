...
| 9 | Legacy (4 purposes) | **in progress** | Committing **per purpose** instead of the plan's single `refactor: deepen legacy page data modules`, so an interrupted run loses at most one purpose. **Devices** (`refactor: stream legacy device regions`) **Endpoints** (`refactor: stream legacy endpoint regions`) and **Health** (`refactor: stream legacy health regions`) done: shared primitives moved `src/lib/legacy-ui/{query,serialize}.ts` → `src/lib/legacy/`, device filters → `src/lib/legacy/devices/filters.ts`; cached reads tagged `legacy-devices:all`; every legacy ingestion now expires that tag through `handleLegacyIngestRequest` (a cache-expiry failure cannot fail a committed ingest). Health's drawer moved off its Server Action to `GET /api/legacy/health/history` and `src/actions/legacy-health.ts` is deleted (the guard's `obsoletePaths` keeps it deleted). **Remaining: interfaces** — still has `src/actions/legacy-interfaces.ts` to replace the same way. |
