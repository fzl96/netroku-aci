...
| 9 | Legacy (4 purposes) | **in progress** | Committing **per purpose** instead of the plan's single `refactor: deepen legacy page data modules`, so an interrupted run loses at most one purpose. **Devices done** (`refactor: stream legacy device regions`): shared primitives moved `src/lib/legacy-ui/{query,serialize}.ts` → `src/lib/legacy/`, device filters → `src/lib/legacy/devices/filters.ts`; cached reads tagged `legacy-devices:all`; every legacy ingestion now expires that tag through `handleLegacyIngestRequest` (a cache-expiry failure cannot fail a committed ingest). **Remaining: endpoints, health, interfaces** — health and interfaces still have read-only Server Actions (`src/actions/legacy-health.ts`, `src/actions/legacy-interfaces.ts`) that need an authenticated read route, like the interface-health drawer. |
