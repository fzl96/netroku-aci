...
const STATUS = { unauthorized: 401, 'host-not-found': 404, 'in-progress': 409, 'sync-failed': 502 } as const
export async function POST(request: Request) {
  ...
  if (!input.username?.trim() || !input.password) return Response.json({ error: 'username and password are required' }, { status: 400 })
  try {
  ...
  } catch { return Response.json({ error: 'Failed to finalize EPG resync' }, { status: 500 }) }
}
