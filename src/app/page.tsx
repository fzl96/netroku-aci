...
function ThemeToggle({ className }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={cn(
        'group relative inline-flex h-9 w-9 items-center justify-center rounded-full border border-border bg-card/60 text-foreground/80 backdrop-blur transition-colors hover:border-foreground/30 hover:text-foreground',
        className,
      )}
    >
...
function GridBackdrop() {
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 -z-10 overflow-hidden">
      ...
      />
      {/* Bottom horizon line */}
      <div className="bg-border-light absolute right-0 bottom-0 left-0 h-px" />
    </div>
  )
}
...
function TopNav() {
  return (
    <header className="border-border-light sticky top-0 z-50 border-b bg-background/70 backdrop-blur-xl">
  ...
  return (
    <div className="relative">
      ...
      <div className="relative overflow-hidden rounded-xl border border-border bg-card shadow-[0_1px_0_0_var(--border-light),0_24px_60px_-30px_color-mix(in_oklch,var(--foreground)_25%,transparent)]">
        {/* Window chrome */}
        <div className="border-border-light flex items-center justify-between border-b bg-muted/50 px-4 py-2.5">
          ...
          <span className="text-success-text inline-flex items-center gap-1 rounded-full border border-success-border bg-success-bg px-1.5 py-0.5 text-[10px] font-medium">
            <IconCheck size={10} stroke={2.5} />
            APIC connected
          </span>
        </div>
        {/* Column header */}
        <div className="border-border-light grid grid-cols-[1.2fr_1.4fr_0.6fr_1.4fr_0.4fr] gap-3 border-b bg-background/40 px-4 py-2 font-mono text-[10px] tracking-wider text-faint uppercase">
          <span>tenant</span>
          <span>epg</span>
          <span>vlan</span>
          <span>port</span>
        ...
        {/* Footer */}
        <div className="border-border-light flex items-center justify-between border-t bg-muted/40 px-4 py-2.5 text-[11px]">
      ...
      {/* Floating note */}
      <div
        className="absolute -right-3 -bottom-4 hidden rounded-md border border-border bg-card px-2.5 py-1.5 font-mono text-[10px] text-muted-foreground shadow-lg sm:flex sm:items-center sm:gap-1.5"
        ...
        <div className="grid items-center gap-16 lg:grid-cols-[1.05fr_0.95fr]">
          ...
          <div className="flex flex-col">
            ...
            >
              ...
              <span className="font-mono tracking-wider uppercase">v1.0 · cisco aci toolkit</span>
            </div>
            <h1
            ...
            >
              Ship fabric policy.
              <br />
              <span className="text-muted-foreground italic">From a </span>
            ...
            >
              Bulk deploy and rollback Cisco ACI access policy, bridge domains, EPGs, and static
              port bindings — without ever opening the APIC GUI. Validate first. Deploy in parallel.
              Undo with the same file.
            </p>
            <div
            ...
            {/* Mini stats row */}
            <dl
              className="border-border-light mt-12 grid grid-cols-3 gap-6 border-t pt-6"
            ...
            >
              {[
                ...
                <div key={s.l}>
                  ...
                  <dd className="mt-1 font-mono text-[10.5px] tracking-wider text-faint uppercase">
                    {s.l}
                  </dd>
                </div>
              ))}
...
function Workflows() {
  return (
    <section id="workflows" className="border-border-light relative border-t bg-background">
      <div className="mx-auto max-w-6xl px-6 py-24 sm:py-28">
        <div className="mb-10 flex items-end justify-between gap-8">
          <div>
            <p className="font-mono text-[10.5px] tracking-[0.18em] text-muted-foreground uppercase">
              · Workflows
            </p>
            <h2 className="mt-3 max-w-xl font-serif text-3xl tracking-tight text-foreground sm:text-4xl">
              Every workflow ships with a{' '}
              <span className="text-muted-foreground italic">rollback</span>.
            </h2>
          </div>
          <p className="hidden max-w-xs text-[13px] text-muted-foreground md:block">
            Six bulk operations covering the policy surface area you actually touch on Monday
            morning.
          </p>
        </div>
        ...
        <ul className="grid grid-cols-1 gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-2 lg:grid-cols-3">
          {WORKFLOWS.map((w) => (
            <li key={w.n} className="group relative flex flex-col gap-3 bg-card p-7">
              <div className="flex items-baseline justify-between">
                <span className="font-mono text-[10.5px] tracking-wider text-faint">{w.n}</span>
                <IconArrowUpRight
                  size={14}
                  stroke={1.75}
                  className="text-faint opacity-0 transition-all group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-foreground group-hover:opacity-100"
                />
              </div>
              <h3 className="font-serif text-xl text-foreground">{w.name}</h3>
              <p className="text-[13.5px] leading-relaxed text-muted-foreground">{w.desc}</p>
            </li>
          ))}
        </ul>
      </div>
...
function HowItWorks() {
  return (
    <section id="how" className="border-border-light relative border-t bg-muted/30">
      <div className="mx-auto max-w-6xl px-6 py-24 sm:py-28">
        <div className="mb-14 flex items-end justify-between">
          <div>
            ...
            <h2 className="mt-3 max-w-2xl font-serif text-3xl tracking-tight text-foreground sm:text-4xl">
              Three steps. <span className="text-muted-foreground italic">No surprises.</span>
            </h2>
          </div>
        </div>
        <ol className="bg-border-light grid gap-px overflow-hidden rounded-2xl border border-border sm:grid-cols-3">
          {STEPS.map((s, i) => (
            <li key={s.n} className="relative flex flex-col gap-5 bg-card p-8">
              <div className="flex items-center justify-between">
                <span className="font-serif text-3xl text-muted-foreground/70 italic">{s.n}</span>
                <s.icon size={18} stroke={1.5} className="text-foreground/80" />
              </div>
              ...
              {i < STEPS.length - 1 && (
                <span
                  aria-hidden
                  className="absolute top-1/2 -right-2 hidden h-4 w-4 -translate-y-1/2 items-center justify-center rounded-full border border-border bg-card text-muted-foreground sm:flex"
                >
...
function Safety() {
  ...
  return (
    <section id="safety" className="border-border-light relative border-t bg-background">
      <div className="mx-auto grid max-w-6xl gap-16 px-6 py-24 sm:py-28 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="lg:sticky lg:top-24 lg:self-start">
          ...
          <h2 className="mt-3 font-serif text-3xl tracking-tight text-foreground sm:text-4xl">
            Built for <span className="text-muted-foreground italic">the change window</span> you
            actually have.
          </h2>
          <p className="mt-6 max-w-md text-[14px] leading-relaxed text-muted-foreground">
            Every action is reversible. Every error is local. Nothing leaves the browser until
            you&rsquo;ve seen the validation pass.
          </p>
        </div>

        <ul className="divide-border-light border-border-light divide-y border-y">
          {bullets.map((b, i) => (
            <li key={i} className="flex items-start gap-5 py-5">
              <span className="mt-0.5 font-mono text-[10.5px] tracking-wider text-faint">
                0{i + 1}
              </span>
              <span className="flex-1 text-[14px] leading-relaxed text-foreground/85">{b}</span>
              <IconCheck size={14} stroke={2} className="mt-1 shrink-0 text-primary" />
            </li>
          ))}
        </ul>
      </div>
...
function CTA() {
  return (
    <section className="border-border-light relative border-t bg-muted/40">
      <div className="mx-auto max-w-6xl px-6 py-24 text-center sm:py-32">
        ...
        <h2 className="mx-auto mt-4 max-w-3xl font-serif text-4xl leading-[1.05] tracking-tight text-foreground sm:text-5xl">
          ...
          <span className="text-muted-foreground italic">Start shipping policy.</span>
        </h2>
...
function Footer() {
  return (
    <footer className="border-border-light border-t bg-background">
      <div className="mx-auto flex max-w-6xl flex-col gap-6 px-6 py-10 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-6">
          <LogoMark />
          <span className="font-mono text-[10.5px] tracking-wider text-faint uppercase">
        ...
        <div className="flex items-center gap-6 text-[12.5px] text-muted-foreground">
          ...
          <span className="font-mono text-faint">© {new Date().getFullYear()}</span>
        </div>
      </div>
    </footer>
  )
