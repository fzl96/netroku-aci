...
import { buildLegacyHealthDeviceWhere, serializeLegacyHealthSample } from '@/lib/legacy-ui/health'
...
export default async function LegacyHealthPage({ searchParams }: { searchParams: Promise<{ query?: string; site?: string; page?: string; pageSize?: string }> }) {
  ...
  const [devices, total, monitored, samples, logs, latest, siteOptions] = await Promise.all([
    prisma.legacyDevice.findMany({ where, orderBy: { lastHealthSyncAt: 'desc' }, skip: (page - 1) * pageSize, take: pageSize, select: { id: true, hostname: true, site: true, managementIp: true, healthSamples: { orderBy: { collectedAt: 'desc' }, take: 1 } } }),
  ...
  return <LegacyHealthClient rows={rows} total={total} page={page} pageSize={pageSize} query={params.query ?? ''} site={params.site ?? ''} siteOptions={siteOptions.map(row => row.site)} summaries={{ devices: monitored, samples, logs, latest: latest?.collectedAt.toISOString() ?? null }} />
}
