...
export function LegacyHealthClient({ rows, total, page, pageSize, query, site, siteOptions, summaries }: { rows: LegacyHealthRow[]; total: number; page: number; pageSize: LegacyPageSize; query: string; site: string; siteOptions: string[]; summaries: { devices: number; samples: number; logs: number; latest: string | null } }) {
  const router = useRouter(); const pathname = usePathname(); const searchParams = useSearchParams()
  const [search, setSearch] = useState(query); const [selected, setSelected] = useState<LegacyHealthRow | null>(null)
  function apply(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); const params = new URLSearchParams(searchParams.toString()); for (const key of ['query', 'site']) { const value = String(form.get(key) ?? '').trim(); if (value) params.set(key, value); else params.delete(key) } params.set('page', '1'); router.push(`${pathname}?${params}`) }
