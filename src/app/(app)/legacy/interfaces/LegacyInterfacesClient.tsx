...
export function LegacyInterfacesClient({
  ...
  filters: { query: string; site: string; device: string; admin: string; oper: string; presence: LegacyInterfacePresence }
  options: {
    sites: string[]
  ...
  function apply(event: FormEvent<HTMLFormElement>) {
    ...
    for (const key of ['query', 'site', 'device', 'admin', 'oper', 'presence']) {
      const value = String(form.get(key) ?? '').trim()
      if (value && !(key === 'presence' && value === 'present')) params.set(key, value)
  ...
  return <LegacyPageShell title="Legacy Interfaces" description="Current interface inventory, exact counters, and historical trends">
    ...
    {rows.length === 0 ? <LegacyEmptyState icon={<IconPlugConnected size={24} />} title="No legacy interfaces found" description="Run legacy_sync.py monitor or all to collect interfaces, or clear the current filters." /> : <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      <div className="hidden max-h-[calc(100vh-17rem)] overflow-auto md:block"><table className="w-full text-xs"><thead><tr>{['Device', 'Interface', 'Description', 'IP address', 'Admin', 'Operational', 'Speed', 'Input errors', 'Output errors', 'CRC errors', 'Collected'].map(label => <th key={label} className={DENSE_TABLE_HEAD_CLS}>{label}</th>)}</tr></thead><tbody>{rows.map(row => <tr key={row.id} onClick={() => setSelected(row)} className="cursor-pointer border-b border-border/70 hover:bg-muted/60"><td className="px-4 py-3 font-semibold text-foreground">{row.hostname}<div className="text-[10px] font-normal text-faint">{row.site}</div></td><td className="whitespace-nowrap px-4 py-3 font-mono text-foreground">{row.ifName}{!row.present && <div className="font-sans text-[10px] text-faint">Absent</div>}</td><td className="max-w-52 truncate px-4 py-3 text-subtle">{row.description || '—'}</td><td className="whitespace-nowrap px-4 py-3 font-mono text-subtle">{row.ipAddress ? `${row.ipAddress}${row.prefixLength === null ? '' : `/${row.prefixLength}`}` : '—'}</td><td className="px-4 py-3">{stateBadge(row.adminSt, row.present)}</td><td className="px-4 py-3">{stateBadge(row.operSt, row.present)}</td><td className="whitespace-nowrap px-4 py-3 text-subtle">{row.sample?.speed || row.speed || '—'}</td><td className="px-4 py-3 text-right font-mono text-subtle">{exactCounter(row.sample?.inputErrors ?? null)}</td><td className="px-4 py-3 text-right font-mono text-subtle">{exactCounter(row.sample?.outputErrors ?? null)}</td><td className="px-4 py-3 text-right font-mono text-subtle">{exactCounter(row.sample?.crcErrors ?? null)}</td><td className="whitespace-nowrap px-4 py-3 text-subtle">{row.sample ? new Date(row.sample.collectedAt).toLocaleString() : 'No samples'}</td></tr>)}</tbody></table></div>
      <div className="space-y-2 p-3 md:hidden">{rows.map(row => <DataCard key={row.id} onClick={() => setSelected(row)}><DataCardHeader trailing={stateBadge(row.operSt, row.present)}><DataCardTitle>{row.hostname} · {row.ifName}</DataCardTitle></DataCardHeader><DataCardBody><DataCardRow label="Site" value={row.site} /><DataCardRow label="Description" value={row.description || 'Not reported'} /><DataCardRow label="Errors (in / out / CRC)" value={`${exactCounter(row.sample?.inputErrors ?? null)} / ${exactCounter(row.sample?.outputErrors ?? null)} / ${exactCounter(row.sample?.crcErrors ?? null)}`} /></DataCardBody></DataCard>)}</div>
