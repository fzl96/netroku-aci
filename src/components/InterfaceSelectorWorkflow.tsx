...
const MODE_CONFIG: Record<Mode, {
  ...
  deploy: {
    ...
    step3: { n: 3, label: 'Deploy', sub: 'Push selectors to fabric' },
    queuedNoun: 'queued',
  },
  rollback: {
    pageBadge: 'Rollback',
    pageSubtitle: 'Remove interface selectors from the ACI fabric using a CSV',
    step3: { n: 3, label: 'Rollback', sub: 'Remove selectors from fabric' },
...
export function InterfaceSelectorWorkflow({ mode }: InterfaceSelectorWorkflowProps) {
  ...
  const [csvRows, setCsvRows]       = useState<ParsedSelectorRow[]>([])
  const [deployRows, setDeployRows] = useState<ParsedSelectorRow[]>([])
  ...
  function handleUploaded(rows: ParsedSelectorRow[]) {
    setCsvRows(rows)
    setStep(2)
  }

  function handleDeploy(rows: ParsedSelectorRow[]) {
    setDeployRows(rows)
    setStep(3)
  }
          ...
          <div className="max-w-md mx-auto animate-fade-up">
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-[var(--text-subtle)] mt-2 max-w-sm mx-auto">
                Enter APIC controller credentials to begin {mode === 'deploy' ? 'deploying' : 'rolling back'} interface selectors.
              </p>
            </div>
