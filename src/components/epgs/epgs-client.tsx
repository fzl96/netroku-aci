...
function useNavigation() {
  ...
  if (!value) throw new Error('EPG navigation requires EpgsClient')
  return value
}
...
export function EpgsClient({ actions, children }: { actions: ReactNode; children: ReactNode }) {
  ...
  const navigation = {
    isPending,
    navigate: (next: string) => startTransition(() => router.replace(next)),
...
export function EpgHeaderActionsClient({
  params,
  hosts,
  hostTotal,
  filteredTotal,
  ...
  return (
    <div className="flex w-full items-center gap-2 md:w-auto">
      <select
        value={params.hostId}
        onChange={(event) =>
          navigate(event.target.value ? `/epgs?apic=${event.target.value}` : '/epgs')
        }
        disabled={isPending}
...
export function EpgOverviewClient({
  params,
  overview,
}: {
  params: EpgPageParams
  ...
  function go(overrides: Overrides) {
    navigate(url(params, overrides))
  }
  function searchChange(value: string) {
    setSearch(value)
    if (timer.current) clearTimeout(timer.current)
    timer.current = setTimeout(() => go({ query: value, page: 1 }), 300)
  }
      ...
      <div className="flex w-full min-w-0 flex-wrap items-center gap-2 md:w-auto">
        <div className="flex overflow-hidden rounded-lg border border-border">
          {(
            ...
            <button
              ...
              disabled={isPending}
              className={`px-3 py-2 text-xs font-semibold ${params.view === view ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}
            >
              {label}
            </button>
          ))}
        ...
        <div className="relative min-w-[140px] flex-1 md:w-56 md:flex-none">
          ...
          <input
            ...
            placeholder={
              params.view === 'epg' ? 'Search EPG, tenant, BD…' : 'Search node, port, EPG…'
            }
...
export function EpgResultsClient({
  params,
  results,
}: {
  params: EpgPageParams
  ...
  function go(overrides: Overrides) {
    navigate(url(effective, overrides))
  }
  const selectedEpg =
                ...
                go({
                  pageSize:
                    event.target.value === 'all'
                      ? 'all'
                      : (Number(event.target.value) as EpgPageSize),
                  page: 1,
                })
              }
              disabled={isPending}
