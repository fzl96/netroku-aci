...
export function InterfaceRegionError({
  region,
  compact = false,
}: {
  region: 'filters' | 'summary' | 'trend' | 'results'
  compact?: boolean
}) {
  ...
  return (
    <div
      role="alert"
      className={[
        'flex items-center justify-between gap-4 border border-border bg-card',
        compact ? 'rounded-lg px-3 py-2' : 'rounded-2xl px-4 py-5',
      ].join(' ')}
    >
      <div>
        <p className="text-sm font-medium text-foreground">Could not load interface {region}</p>
        {!compact && (
