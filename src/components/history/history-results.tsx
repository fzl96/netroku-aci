...
import type { HistoryLoadState, HistoryPageData, HistoryResultsPayload } from '@/lib/history/query'
...
function HistoryResultsContent({
  params,
  results,
}: {
...
export function HistoryResults({
  ...
  dataPromise: Promise<HistoryLoadState<HistoryResultsPayload>>
}) {
  const state = use(dataPromise)
  if (state.kind === 'unauthorized') return <HistoryRegionError />

  const { params, results } = state.data
  return (
    <HistoryResultsContent
      key={`${params.query}:${params.action}:${results.page}`}
      params={params}
      results={results}
    />
  )
