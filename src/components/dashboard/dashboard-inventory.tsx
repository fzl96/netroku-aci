import type { DashboardEndpointData, DashboardHost, DashboardNodeData } from '@/lib/dashboard/query'
...
export async function DashboardInventory({
  ...
  try {
    ;[hosts, endpoints, nodes] = await Promise.all([hostsPromise, endpointsPromise, nodesPromise])
  } catch {
              ...
              hostSummaries.map((host) => (
                <tr key={host.id}>
                  ...
                  <td className="px-4 py-3 text-foreground">
                    {formatDashboardNumber(host.activeEndpoints)}{' '}
                  ...
                  <td className="px-4 py-3">
                    ...
                    {host.failedHardware > 0 ? (
                      <span className="ml-2 text-xs text-error">
                        {formatDashboardNumber(host.failedHardware)} failed HW
                      </span>
                    ) : null}
                  </td>
                  <td className="py-3 pl-4 text-xs text-muted-foreground">
                    {formatRelativeFreshness(host.freshest)}
                  </td>
                </tr>
              ))
            ) : (
          ...
          hostSummaries.map((host) => (
            <DataCard key={host.id}>
              ...
              <DataCardBody>
                <DataCardRow
                  label="Endpoints"
                  value={`${formatDashboardNumber(host.activeEndpoints)} active`}
                />
                <DataCardRow
                  label="Nodes"
                  value={
                    host.nodesTotal === 0 ? '—' : `${host.nodesOnline}/${host.nodesTotal} online`
                  }
                />
