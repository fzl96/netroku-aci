...
export async function DashboardPosture({
  ...
  const headlineStats: HeadlineStat[] = [
    ...
    },
    {
      ...
      tone:
        nodes.failedHardware > 0 ? 'critical' : nodes.hardwareTotal === 0 ? 'unknown' : 'healthy',
    },
    {
      ...
      <div className="grid lg:grid-cols-[1.1fr_1.9fr]">
        <div className="p-5 md:p-6">
          <div className="flex items-center gap-2">
            <span
              className={`inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1 text-xs font-semibold ${dashboardToneSurfaceClass(posture.tone)}`}
            >
          ...
          <div className="mt-6">
            <p className="text-xs font-medium text-muted-foreground uppercase">Global posture</p>
            <h2 className="mt-2 max-w-xl font-serif text-3xl font-semibold tracking-normal text-foreground md:text-4xl">
              {posture.detail}
            </h2>
            <p className="mt-4 max-w-lg text-sm leading-6 text-muted-foreground">
              Interface counters, endpoint inventory, node state, and hardware are summarized here
              before operators drill into the detailed pages.
            </p>
          </div>
        </div>
        ...
        <div className="grid border-t border-border sm:grid-cols-2 lg:border-t-0 lg:border-l">
          {headlineStats.map((stat, index) => (
            ...
            >
              ...
              <p className={`mt-4 text-3xl font-semibold ${dashboardToneTextClass(stat.tone)}`}>
                {stat.value}
              </p>
