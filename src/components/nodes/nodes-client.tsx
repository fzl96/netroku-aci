...
export function NodeHeaderActionsClient({
  params,
  hosts,
  lastNodeSyncAt,
}: {
  ...
  return (
    <div className="flex w-full items-center gap-2 md:w-auto">
      <span className="hidden text-xs text-subtle lg:inline">
        Last synced {fmtRelative(lastNodeSyncAt)}
      </span>
      <select
        value={params.hostId}
        onChange={(event) =>
          navigate(event.target.value ? `/nodes?apic=${event.target.value}` : '/nodes')
        }
        disabled={isPending}
...
export function NodeResultsClient({
  params,
  results,
}: {
  params: NodePageParams
