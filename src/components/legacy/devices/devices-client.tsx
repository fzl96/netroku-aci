...
export function LegacyDeviceFiltersClient({
  ...
  function applyFilters(event: FormEvent<HTMLFormElement>) {
    ...
    router.push(buildLegacyDevicePageUrl({
      ...params,
      query: read('query'),
      site: read('site'),
  ...
  return (
    <form onSubmit={applyFilters} className="flex flex-col gap-2 sm:flex-row sm:items-center">
      ...
      <select name="site" defaultValue={params.site} aria-label="Filter by site" className="rounded-lg border border-border bg-muted px-3 py-1.5 text-xs text-foreground">
        <option value="">All sites</option>
        {siteOptions.map(value => <option key={value} value={value}>{value}</option>)}
      </select>
      <select name="deviceType" defaultValue={params.deviceType} aria-label="Filter by device type" className="rounded-lg border border-border bg-muted px-3 py-1.5 text-xs text-foreground">
        <option value="">All device types</option>
        {typeOptions.map(value => <option key={value} value={value}>{value}</option>)}
      </select>
      <select name="sort" defaultValue={params.sort} aria-label="Sort devices" className="rounded-lg border border-border bg-muted px-3 py-1.5 text-xs text-foreground">
        {LEGACY_DEVICE_SORTS.map(value => <option key={value} value={value}>{SORT_LABELS[value]}</option>)}
      </select>
      <select name="dir" defaultValue={params.direction} aria-label="Sort direction" className="rounded-lg border border-border bg-muted px-3 py-1.5 text-xs text-foreground">
