...
export function LegacyHealthDrawer({
  selected,
  onClose,
}: {
  selected: LegacyHealthRow | null
  onClose: () => void
}) {
  ...
  const requestKey = selected ? `${selected.deviceId}:${range}:${samplePage}:${logPage}` : null
  ...
  function changeRange(next: LegacyRange) {
    setRange(next)
    setSamplePage(1)
    setLogPage(1)
  }
    ...
    >
      <SheetContent className="w-full overflow-y-auto data-[side=right]:sm:max-w-3xl">
        <SheetHeader className="border-b border-border">
          <SheetTitle className="font-serif">{selected?.hostname ?? 'Health history'}</SheetTitle>
          <SheetDescription>{selected?.site} · collected measurements and logs</SheetDescription>
        </SheetHeader>
        <div className="space-y-5 px-4 pb-8">
          <div className="inline-flex rounded-lg border border-border bg-muted p-0.5">
            {(['24h', '7d', '30d', 'all'] as LegacyRange[]).map((value) => (
              <button
                key={value}
                onClick={() => changeRange(value)}
                className={`rounded-md px-3 py-1.5 text-xs ${range === value ? 'bg-card font-semibold text-foreground shadow-sm' : 'text-subtle'}`}
              >
                {value}
              </button>
            ))}
          ...
          {failed ? (
            <div className="rounded-xl border border-error/30 bg-error/5 p-4 text-xs text-error">
              Could not load health history. Change the range to retry.
            </div>
          ) : loading ? (
            <div className="py-20 text-center text-xs text-subtle">Loading history…</div>
          ) : (
            data && (
              <>
                ...
                <section>
                  ...
                  <div className="overflow-x-auto rounded-xl border border-border">
                    <table className="w-full text-xs">
                      <thead className="bg-muted text-left text-faint">
                        <tr>
                          {['Collected', 'CPU', 'Memory', 'Storage', 'Temp', 'Fans', 'PSUs'].map(
                            (v) => (
                      ...
                      <tbody>
                        {data.samples.map((row) => (
                          <tr key={row.id} className="border-t border-border">
                            ...
                            <td className="px-3 py-2">{metric(row.cpuPercent)}</td>
                            <td className="px-3 py-2">{metric(row.memoryPercent)}</td>
                            <td className="px-3 py-2">{metric(row.storagePercent)}</td>
                            <td className="px-3 py-2">{metric(row.temperatureCelsius, '°C')}</td>
                            <td className="px-3 py-2">{row.fanStatuses.join(', ') || '—'}</td>
                            <td className="px-3 py-2">{row.psuStatuses.join(', ') || '—'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    ...
                    {data.logs.length === 0 ? (
                      ...
                      data.logs.map((log) => (
                        <div key={log.id} className="rounded-xl border border-border bg-card p-3">
                          <div className="flex justify-between gap-3 text-[10px] text-faint">
                            <span>{log.severity || 'UNCLASSIFIED'}</span>
                            <time>{new Date(log.eventAt || log.collectedAt).toLocaleString()}</time>
                          </div>
                          <p className="mt-1 text-xs text-foreground">{log.message}</p>
                          <details className="mt-2 text-[10px] text-subtle">
                            <summary>Raw event</summary>
                            <pre className="mt-1 break-words whitespace-pre-wrap">{log.raw}</pre>
                          </details>
                        </div>
                      ))
                    )}
...
function Pager({
  ...
  return (
    <div className="mt-2 flex items-center justify-end gap-2 text-xs text-subtle">
      <button
        disabled={page <= 1}
