...
export function LegacyHealthFiltersClient({
  ...
  function apply(event: FormEvent<HTMLFormElement>) {
    ...
    router.push(buildLegacyHealthPageUrl({
      ...params,
      query: read('query'),
      site: read('site'),
  ...
  return (
    <form onSubmit={apply} className="flex flex-col gap-2 sm:flex-row">
      <div className="relative flex-1 sm:max-w-xs">
        <IconSearch size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-faint" />
        <input name="query" value={search} onChange={event => setSearch(event.target.value)} placeholder="Search device or site…" className={SEARCH_INPUT_CLS} />
      </div>
      <select name="site" defaultValue={params.site} aria-label="Filter by site" className={SELECT_CLS}><option value="">All sites</option>{siteOptions.map(value => <option key={value}>{value}</option>)}</select>
      <select name="sort" defaultValue={params.sort} aria-label="Sort health" className={SELECT_CLS}>{LEGACY_HEALTH_SORTS.map(value => <option key={value} value={value}>{SORT_LABELS[value]}</option>)}</select>
  ...
  return (
    <>
      <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        <div className="hidden max-h-[calc(100vh-17rem)] overflow-auto md:block"><table className="w-full text-xs"><thead><tr>{['Device', 'Site', 'Uptime', 'CPU', 'Memory', 'Storage', 'Temperature', 'Fans', 'PSUs', 'Collected'].map(value => <th key={value} className={DENSE_TABLE_HEAD_CLS}>{value}</th>)}</tr></thead><tbody>{rows.map(row => <tr key={row.deviceId} onClick={() => setSelected(row)} className="cursor-pointer border-b border-border/70 hover:bg-muted/60"><td className="px-4 py-3 font-semibold">{row.hostname}<div className="font-mono text-[10px] font-normal text-faint">{row.managementIp}</div></td><td className="px-4 py-3 text-subtle">{row.site}</td><td className="px-4 py-3 text-subtle">{row.sample.uptime || '—'}</td><td className="px-4 py-3">{metric(row.sample.cpuPercent)}</td><td className="px-4 py-3">{metric(row.sample.memoryPercent)}</td><td className="px-4 py-3">{metric(row.sample.storagePercent)}</td><td className="px-4 py-3">{metric(row.sample.temperatureCelsius, '°C')}</td><td className="px-4 py-3">{legacyStatusText(row.sample.fanStatuses)}</td><td className="px-4 py-3">{legacyStatusText(row.sample.psuStatuses)}</td><td className="whitespace-nowrap px-4 py-3 text-subtle">{new Date(row.sample.collectedAt).toLocaleString()}</td></tr>)}</tbody></table></div>
        <div className="space-y-2 p-3 md:hidden">{rows.map(row => <DataCard key={row.deviceId} onClick={() => setSelected(row)}><DataCardHeader trailing={<span className="text-[10px] text-faint">{row.site}</span>}><DataCardTitle>{row.hostname}</DataCardTitle></DataCardHeader><DataCardBody><DataCardRow label="CPU / Memory" value={`${metric(row.sample.cpuPercent)} / ${metric(row.sample.memoryPercent)}`} /><DataCardRow label="Temperature" value={metric(row.sample.temperatureCelsius, '°C')} /><DataCardRow label="Collected" value={new Date(row.sample.collectedAt).toLocaleString()} /></DataCardBody></DataCard>)}</div>
