...
export function LegacyInterfaceDrawer({
  selected,
  onClose,
}: {
  selected: LegacyInterfaceRow | null
  onClose: () => void
}) {
  ...
  const [result, setResult] = useState<{
    key: string
    data: LegacyInterfaceHistory | null
    failed: boolean
  } | null>(null)
      ...
      <SheetContent className="w-full overflow-y-auto data-[side=right]:sm:max-w-3xl">
        <SheetHeader className="border-b border-border">
          <SheetTitle className="font-serif">
            {selected?.hostname ?? 'Interface'} · {selected?.ifName}
          </SheetTitle>
          <SheetDescription>
            {selected?.site} · exact counters and historical deltas
          </SheetDescription>
        </SheetHeader>
        <div className="space-y-5 px-4 pb-8">
          ...
          {failed ? (
            <div className="rounded-xl border border-error/30 bg-error/5 p-4 text-xs text-error">
              Could not load interface history. Change the range to retry.
            </div>
          ) : loading ? (
              ...
              <>
                <section className="grid gap-2 text-xs sm:grid-cols-2">
                  <div className="rounded-xl border border-border bg-muted/40 p-3">
                    <p className="text-[10px] tracking-wide text-faint uppercase">Address</p>
                    <p className="mt-1 font-mono">
                      {data.snapshot.ipAddress
                        ? `${data.snapshot.ipAddress}${data.snapshot.prefixLength === null ? '' : `/${data.snapshot.prefixLength}`}`
                        : 'Not assigned'}
                    </p>
                  </div>
                  <div className="rounded-xl border border-border bg-muted/40 p-3">
                    <p className="text-[10px] tracking-wide text-faint uppercase">Current state</p>
                    <p className="mt-1">
                      Admin {data.snapshot.adminSt || 'unknown'} · Oper{' '}
                      {data.snapshot.operSt || 'unknown'} ·{' '}
                      {data.snapshot.present ? 'present' : 'absent'}
                    </p>
                  </div>
                </section>
                      ...
                      <thead className="bg-muted text-left text-faint">
                        <tr>
                          {[
                            ...
                            <th key={value} className="px-3 py-2 whitespace-nowrap">
                              {value}
                            </th>
                          ))}
                        </tr>
                      ...
                      <tbody>
                        {data.samples.map((sample) => (
                          <tr key={sample.id} className="border-t border-border">
                            ...
                            <td className="px-3 py-2">{sample.adminSt || '—'}</td>
                            <td className="px-3 py-2">{sample.operSt || '—'}</td>
                            <td className="px-3 py-2 whitespace-nowrap">{sample.speed || '—'}</td>
