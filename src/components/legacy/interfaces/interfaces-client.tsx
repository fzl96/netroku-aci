...
type Navigation = { isPending: boolean; navigate: (url: string) => void }
const NavigationContext = createContext<Navigation | null>(null)

function useLegacyInterfaceNavigation(): Navigation {
  const value = useContext(NavigationContext)
  if (!value) throw new Error('Legacy interface navigation requires LegacyInterfacesFrame')
  return value
}

/** The controls and the table stream in separately but share one URL, so they
 *  also share a single transition and show one coherent pending state. */
export function LegacyInterfacesFrame({ children }: { children: ReactNode }) {
...
export function LegacyInterfaceFiltersClient({
  state,
  devices,
}: {
  state: LegacyInterfaceListState
  ...
  const { isPending, navigate } = useLegacyInterfaceNavigation()
...
export function LegacyInterfaceResultsClient({
  state,
  results,
}: {
  state: LegacyInterfaceListState
