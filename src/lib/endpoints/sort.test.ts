...
describe('sortEndpointRows', () => {
  it('sorts dates and status without mutating the input', () => {
    ...
    expect(sortEndpointRows(rows, 'lastSeenAt', 'asc').map((row) => row.id)).toEqual([
      'active',
      'historical',
      'missing',
    ])
    expect(sortEndpointRows(rows, 'status', 'asc').map((row) => row.id)).toEqual([
      'active',
      'historical',
      'missing',
    ])
...
describe('sortPortRows', () => {
  it('sorts counts numerically and interfaces naturally', () => {
    ...
    expect(sortPortRows(rows, 'interface', 'asc').map((row) => row.interface)).toEqual([
      'eth1/2',
      'eth1/10',
    ])
    expect(sortPortRows(rows, 'endpointCount', 'desc').map((row) => row.interface)).toEqual([
      'eth1/2',
      'eth1/10',
    ])
  })
