...
export function sortBindingRows<T extends { node: string; port: string }>(rows: T[]): T[] {
  return [...rows].sort(
    (a, b) => NATURAL_COLLATOR.compare(a.node, b.node) || NATURAL_COLLATOR.compare(a.port, b.port),
  )
}
...
export function groupBindingsByPort(bindings: EpgPortBinding[]): EpgPortSummary[] {
  ...
  for (const binding of bindings) {
    ...
    const summary = map.get(id) ?? {
      ...
      node,
      port,
      pathType: binding.pathType || 'port',
      epgCount: 0,
      tenants: [],
      encaps: [],
      modes: [],
    ...
    summary.epgCount += 1
    if (binding.epg.tenant && !summary.tenants.includes(binding.epg.tenant))
      summary.tenants.push(binding.epg.tenant)
