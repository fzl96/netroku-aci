...
describe('EPG mutation boundary', () => {
  it('shares sync semantics and immediately invalidates exact tags', async () => {
    ...
    expect(
      await mutation.resyncEpgInventory({ apicHostId: 'h1', username: ' admin ', password: 'pw' }),
  ...
  it('does not let a failed audit skip invalidation', async () => {
    ...
    const result = await createEpgMutation(deps).resyncEpgInventoryForScheduler({
      apicHostId: 'h1',
      hostName: 'Fabric',
      host: 'apic.local',
      username: 'u',
  ...
  it('audits scheduler failure and rethrows it', async () => {
    const deps = dependencies()
    deps.resyncEpgs.mockImplementation(async () => {
      throw new Error('offline')
    })
    await expect(
      createEpgMutation(deps).resyncEpgInventoryForScheduler({
        apicHostId: 'h1',
        hostName: 'Fabric',
        host: 'apic.local',
        username: 'u',
  ...
  it('maps only a missing session and propagates auth infrastructure failures', async () => {
    ...
    await expect(
      createEpgMutation(missingSessionDeps).resyncEpgInventory({
        apicHostId: 'h1',
        username: 'u',
        password: 'p',
      }),
    ...
    await expect(
      createEpgMutation(infrastructureDeps).resyncEpgInventory({
        apicHostId: 'h1',
        username: 'u',
        password: 'p',
      }),
