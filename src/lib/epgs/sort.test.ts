...
import type { EpgPortBinding } from './sort'
    ...
    const bindings: EpgPortBinding[] = [
      {
        ...
        encap: 'vlan-100',
        mode: 'trunk',
        epg: { name: 'EPG1', tenant: 'T1', appProfile: 'AP1', dn: 'epg1' },
      },
      {
        id: 'b2',
        apicHostId: 'h1',
        ...
        encap: 'vlan-101',
        mode: 'trunk',
        epg: { name: 'EPG2', tenant: 'T1', appProfile: 'AP1', dn: 'epg2' },
      },
      {
        id: 'b3',
        apicHostId: 'h1',
        ...
        encap: 'vlan-200',
        mode: 'access',
        epg: { name: 'EPG3', tenant: 'T2', appProfile: 'AP2', dn: 'epg3' },
      },
    ]

    const result = groupBindingsByPort(bindings)
    expect(result.length).toBe(2)
    expect(result.map((p) => `${p.node} ${p.port}`)).toEqual(['99 eth1/1', '101 eth1/10'])
