...
const BINDING_SELECT = {
  id: true, apicHostId: true, epgId: true, dn: true, pathTDn: true, pod: true,
  node: true, port: true, pathType: true, encap: true, mode: true,
} satisfies Prisma.EpgPathBindingSelect

const EPG_SELECT = {
  id: true, apicHostId: true, dn: true, name: true, tenant: true, appProfile: true,
  description: true, bridgeDomain: true, pcTag: true, preferredGroup: true, isolation: true,
  domains: true, providedContracts: true, consumedContracts: true,
...
export type EpgPagination = { page: number; pageSize: EpgPageSize; total: number; totalPages: number }
...
export class EpgReadError extends Error {
  ...
  constructor() { super('Unauthorized'); this.name = 'EpgReadError' }
}
...
function normalize(filters: EpgFilters = {}): Required<EpgFilters> {
  const values = (items?: string[]) => Array.from(new Set((items ?? []).map(item => item.trim()).filter(Boolean))).sort(NATURAL_COLLATOR.compare)
  return { query: filters.query?.trim() ?? '', tenant: values(filters.tenant), ap: values(filters.ap), node: values(filters.node) }
}

function filterParts(filters: Required<EpgFilters>): string[] {
  return [filters.query, JSON.stringify(filters.tenant), JSON.stringify(filters.ap), JSON.stringify(filters.node)]
}

function nodeConditions(value: string): Prisma.EpgPathBindingWhereInput[] {
  return [{ node: value }, { node: { startsWith: `${value}-` } }, { node: { endsWith: `-${value}` } }]
}

export function buildEpgWhere(apicHostId: string, filters: EpgFilters): Prisma.EpgSnapshotWhereInput {
  ...
  return {
    ...
    ...(query ? { OR: [
      { name: { contains: query, mode: 'insensitive' } }, { tenant: { contains: query, mode: 'insensitive' } },
      { appProfile: { contains: query, mode: 'insensitive' } }, { bridgeDomain: { contains: query, mode: 'insensitive' } },
      { description: { contains: query, mode: 'insensitive' } }, { dn: { contains: query, mode: 'insensitive' } },
    ] } : {}),
  }
}
...
export function buildBindingWhere(apicHostId: string, filters: EpgFilters): Prisma.EpgPathBindingWhereInput {
  const query = filters.query?.trim()
  const epg = {
    ...(filters.tenant?.length ? { tenant: { in: filters.tenant } } : {}),
    ...(filters.ap?.length ? { appProfile: { in: filters.ap } } : {}),
  ...
  return {
    ...
    ...(query ? { OR: [
      { node: { contains: query, mode: 'insensitive' } }, { port: { contains: query, mode: 'insensitive' } },
      { encap: { contains: query, mode: 'insensitive' } }, { epg: { name: { contains: query, mode: 'insensitive' } } },
...
export function expandNodeOptions(values: string[]): string[] {
  return Array.from(new Set(values.flatMap(value => value.split('-')).filter(Boolean))).sort(NATURAL_COLLATOR.compare)
}

async function resolveForRequest(requestedHostId: string): Promise<EpgHostResolution> {
  await authorize()
  const hosts = await prisma.apicHost.findMany({ orderBy: { createdAt: 'desc' }, select: { id: true, name: true, host: true } })
...
function pagination(total: number, requestedPage: number, pageSize: EpgPageSize): EpgPagination {
  ...
  return { page: pageSize === 'all' ? 1 : Math.min(requestedPage, totalPages), pageSize, total, totalPages }
}

export async function getEpgOverview(hostId: string, params: EpgPageParams): Promise<EpgOverviewData> {
  await authorize()
  const filters = normalize({ query: params.query, tenant: params.tenants, ap: params.appProfiles, node: params.nodes })
  return unstable_cache(async () => {
    const hostWhere = { apicHostId: hostId }
    const filteredWhere = { ...buildEpgWhere(hostId, filters), ...epgHasNodeBindingWhere(filters.node) }
    const [host, tenantRows, apRows, nodeRows, hostTotal, filteredTotal] = await Promise.all([
      ...
      prisma.epgSnapshot.findMany({ where: hostWhere, select: { tenant: true }, distinct: ['tenant'], orderBy: { tenant: 'asc' } }),
      prisma.epgSnapshot.findMany({ where: hostWhere, select: { appProfile: true }, distinct: ['appProfile'], orderBy: { appProfile: 'asc' } }),
      prisma.epgPathBinding.findMany({ where: hostWhere, select: { node: true }, distinct: ['node'] }),
      prisma.epgSnapshot.count({ where: hostWhere }), prisma.epgSnapshot.count({ where: filteredWhere }),
    ])
    return {
      hostTotal, filteredTotal, lastEpgSyncAt: host?.lastEpgSyncAt?.toISOString() ?? null,
      choices: {
        tenants: Array.from(new Set(tenantRows.map(row => row.tenant).filter(Boolean))).sort(NATURAL_COLLATOR.compare),
        appProfiles: Array.from(new Set(apRows.map(row => row.appProfile).filter(Boolean))).sort(NATURAL_COLLATOR.compare),
        nodes: expandNodeOptions(nodeRows.map(row => row.node).filter(Boolean)),
      },
    }
  }, ['epgs', 'overview', hostId, ...filterParts(filters)], cacheOptions(hostId))()
}

export async function getEpgResults(params: EpgPageParams): Promise<EpgResultsData> {
  await authorize()
  const filters = normalize({ query: params.query, tenant: params.tenants, ap: params.appProfiles, node: params.nodes })
  return unstable_cache(async (): Promise<EpgResultsData> => {
    if (params.view === 'port') {
