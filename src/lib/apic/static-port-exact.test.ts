...
describe('exact static-port validation', () => {
  it('deduplicates shared nodes and bundle groups while preserving row results', async () => {
  ...
  it('deduplicates identical rollback targets', async () => {
  ...
  it('does not fetch APIC state for an empty review', async () => {
    const { calls, reader } = countingReader()

    expect(await validateDeployRowsExact([], 'apic.local', 'token', reader)).toEqual([])
    expect(await validateRollbackRowsExact([], 'apic.local', 'token', reader)).toEqual([])
