...
describe('classifyPosture', () => {
  test('returns critical when failed hardware exists', () => {
    expect(classifyPosture({ failedHardware: 1, offlineNodes: 0, noisyInterfaces: 0 }).tone).toBe('critical')
  })

  test('returns warning for noisy interfaces without critical blockers', () => {
    expect(classifyPosture({ failedHardware: 0, offlineNodes: 0, noisyInterfaces: 2 }).tone).toBe('warning')
  })

  test('returns healthy when no risk signals are present', () => {
    expect(classifyPosture({ failedHardware: 0, offlineNodes: 0, noisyInterfaces: 0 }).tone).toBe('healthy')
  })
})
...
describe('summarizeInterfaces', () => {
  test('counts interface state and only treats the newest sample as noisy', () => {
    expect(summarizeInterfaces(
      ...
      [
        { interfaceId: 'eth1', sampledAt: '2026-06-15T12:00:00Z', dRxErrors: 0, dTxErrors: 0, dRxDiscards: 0, dTxDiscards: 0, dRxCrcErrors: 0, dRxAlignErrors: 0 },
        { interfaceId: 'eth1', sampledAt: '2026-06-15T11:00:00Z', dRxErrors: 5, dTxErrors: 0, dRxDiscards: 0, dTxDiscards: 0, dRxCrcErrors: 0, dRxAlignErrors: 0 },
        { interfaceId: 'eth2', sampledAt: '2026-06-15T12:00:00Z', dRxErrors: 0, dTxErrors: 0, dRxDiscards: 1, dTxDiscards: 0, dRxCrcErrors: 0, dRxAlignErrors: 0 },
      ],
