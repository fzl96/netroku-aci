...
export async function getDashboardHosts(): Promise<DashboardHost[]> {
  ...
  return unstable_cache(
    ...
    ['dashboard', 'hosts'],
    cacheOptions(['apic-hosts:all', 'endpoints:all', 'interfaces:all', 'nodes:all']),
  )()
}
...
export async function getDashboardEndpoints(): Promise<DashboardEndpointData> {
  ...
  return unstable_cache(
    async () => {
      ...
      const countStatus = (active: boolean) =>
        statusRows
  ...
  return unstable_cache(
    async () => {
      ...
      return {
        ...
        failedPsu: hardware.filter((component) => component.type === 'psu' && !component.healthy)
          .length,
        failedFan: hardware.filter((component) => component.type === 'fan' && !component.healthy)
          .length,
        byHost: Array.from(hostIds)
          .sort()
          .map((hostId) => {
            ...
            return {
              ...
              failedHardware: hardware.filter(
                (component) => component.apicHostId === hostId && !component.healthy,
              ).length,
            }
          }),
      }
