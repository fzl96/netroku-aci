...
describe('resyncScheduleUpdateSchema', () => {
  ...
  it('still validates the interval', () => {
    expect(resyncScheduleUpdateSchema.safeParse({ ...valid, intervalMinutes: 1 }).success).toBe(
      false,
    )
  })

  it('rejects an interval below the floor', () => {
    expect(resyncScheduleUpdateSchema.safeParse({ ...valid, intervalMinutes: 14 }).success).toBe(
      false,
    )
  })

  it('accepts exactly the floor', () => {
    expect(resyncScheduleUpdateSchema.safeParse({ ...valid, intervalMinutes: 15 }).success).toBe(
      true,
    )
  })

  it('accepts exactly the ceiling', () => {
    expect(resyncScheduleUpdateSchema.safeParse({ ...valid, intervalMinutes: 10080 }).success).toBe(
      true,
    )
  })

  it('rejects an interval above the ceiling', () => {
    expect(resyncScheduleUpdateSchema.safeParse({ ...valid, intervalMinutes: 10081 }).success).toBe(
      false,
    )
  })

  it('rejects a non-integer interval', () => {
    expect(resyncScheduleUpdateSchema.safeParse({ ...valid, intervalMinutes: 20.5 }).success).toBe(
      false,
    )
  })
