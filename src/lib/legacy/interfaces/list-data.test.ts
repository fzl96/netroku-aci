...
function sample(
  values: Partial<NonNullable<TestRow['sample']>> = {},
): NonNullable<TestRow['sample']> {
  return {
...
describe('legacy interface list data', () => {
  test('sorts hostname ties by natural interface order', () => {
    ...
    expect(
      sortLegacyInterfaceRows(rows, {
        key: 'hostname',
        direction: 'asc',
        mode: 'delta',
        view: 'all',
      }).map((item) => item.ifName),
    ).toEqual(['Ethernet1/1', 'Ethernet1/2', 'Ethernet1/10'])
  ...
  test('sorts the selected current or delta counter with nulls last', () => {
    const rows = [
      row({
        id: 'raw-100',
        ifName: 'Ethernet1/1',
        sample: sample({ inputErrors: '100', dInputErrors: '1' }),
      }),
      row({
        id: 'delta-10',
        ifName: 'Ethernet1/2',
        sample: sample({ inputErrors: '20', dInputErrors: '10' }),
      }),
    ...
    ).toEqual(['delta-10', 'raw-100', 'missing'])
    expect(
      sortLegacyInterfaceRows(rows, {
        key: 'inputErrors',
        direction: 'desc',
    ...
    ).toEqual(['raw-100', 'delta-10', 'missing'])
    expect(
      sortLegacyInterfaceRows(rows, {
        key: 'inputErrors',
        direction: 'asc',
  ...
  test('sorts CRC view by exact window total', () => {
    const rows = [
      row({
        ...
        ifName: 'Ethernet1/1',
        crcWindowTotal: '9',
        sample: sample({ dCrcErrors: '1000' }),
      }),
      row({
        id: 'large',
        ifName: 'Ethernet1/2',
        crcWindowTotal: '9007199254740993',
        sample: sample({ dCrcErrors: '1' }),
      }),
    ]
