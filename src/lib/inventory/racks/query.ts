...
export async function getRacksBySite(siteId: string): Promise<SafeRackWithDevices[]> {
  ...
  }, ['inventory', 'racks', 'by-site', siteId], { tags: [INVENTORY_TAG], revalidate: INVENTORY_CACHE_SECONDS })())
}

export async function getAllRacksForDropdown(): Promise<RackDropdownOption[]> {
  await authorizeInventoryRead()
  return readInventoryData(() => unstable_cache(() => prisma.rack.findMany({
    select: { id: true, name: true, site: { select: { name: true } } },
    orderBy: { name: 'asc' },
  }), ['inventory', 'racks', 'dropdown'], { tags: [INVENTORY_TAG], revalidate: INVENTORY_CACHE_SECONDS })())
}
