...
function sample(overrides: Partial<InterfaceExportSample> = {}): InterfaceExportSample {
  return {
    ...
    rxErrors: BigInt(3),
    rxDiscards: BigInt(4),
    rxCrcErrors: BigInt(5),
    rxAlignErrors: BigInt(6),
    txBytes: BigInt(7),
    txPkts: BigInt(8),
    txErrors: BigInt(9),
    txDiscards: BigInt(10),
    dRxBytes: BigInt(11),
    dRxErrors: BigInt(12),
    dRxDiscards: BigInt(13),
    dRxCrcErrors: BigInt(14),
    dRxAlignErrors: BigInt(15),
    dTxBytes: BigInt(16),
    dTxErrors: BigInt(17),
    dTxDiscards: BigInt(18),
    interface: {
      node: 'leaf-1',
      ifName: 'eth1/1',
      usage: 'epg',
      description: 'uplink',
      dn: 'topology/pod-1/eth1-1',
    },
    ...overrides,
  }
}
...
describe('buildInterfaceSamplesCsv', () => {
  ...
  it('renders absent deltas as empty cells', () => {
    const csv = buildInterfaceSamplesCsv([
      sample({
        dRxBytes: null,
        dRxErrors: null,
        dRxDiscards: null,
...
describe('interfaceExportFilename', () => {
  it('slugifies the host name and stamps the time', () => {
    expect(interfaceExportFilename('Prod APIC #1', new Date('2026-01-02T03:04:05.678Z'))).toBe(
      'interfaces-prod-apic-1-2026-01-02T03-04-05-678Z',
    )
  })

  it('falls back to a placeholder when the name has no usable characters', () => {
    expect(interfaceExportFilename('***', new Date('2026-01-02T03:04:05.678Z'))).toBe(
      'interfaces-host-2026-01-02T03-04-05-678Z',
    )
  })
})
