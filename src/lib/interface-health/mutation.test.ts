  ...
  const mutation = createInterfaceMutation({
    ...
    findHost: async () =>
      overrides.host === undefined
        ? { id: 'h1', name: 'Fabric', host: 'apic.local' }
        : overrides.host,
    resyncInterfaces: overrides.resync ?? (async () => ({ synced: 12, total: 40 })),
    recordAudit: async (input) => {
      audits.push(input as AuditRecord)
    },
    revalidateTag: (tag) => {
      tags.push(tag)
    },
...
describe('resyncInterfaceInventory', () => {
  it('syncs, audits the acting user, and invalidates the host reads', async () => {
    ...
    expect(await mutation.resyncInterfaceInventory(credentials)).toEqual({
      ok: true,
      synced: 12,
      total: 40,
    })
  ...
  it('reports an unauthenticated caller without touching the cache', async () => {
    const { mutation, audits, tags } = setup({
      session: async () => {
        throw new AuthenticationRequiredError('nope')
      },
    })
    expect(await mutation.resyncInterfaceInventory(credentials)).toEqual({
      ok: false,
  ...
  it('reports a failed sync without invalidating reads', async () => {
    const { mutation, tags } = setup({
      resync: async () => {
        throw new Error('APIC unreachable')
      },
    })
    expect(await mutation.resyncInterfaceInventory(credentials)).toEqual({
      ok: false,
      code: 'sync-failed',
      error: 'Failed to resync interfaces',
    })
    expect(tags).toEqual([])
  })
})
...
describe('resyncInterfaceInventoryForScheduler', () => {
  ...
  it('audits a successful scheduled run and invalidates the host reads', async () => {
    ...
    expect(await mutation.resyncInterfaceInventoryForScheduler(scheduled)).toEqual({
      synced: 12,
      total: 40,
    })
    expect(audits).toEqual([
  ...
  it('audits a failed scheduled run and rethrows for the host runner', async () => {
    ...
    await expect(mutation.resyncInterfaceInventoryForScheduler(scheduled)).rejects.toThrow(
      'APIC unreachable',
    )
    expect(audits).toEqual([
      {
