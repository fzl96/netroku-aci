...
mock.module('@/lib/prisma', () => ({
  prisma: {
    ...
    hardwareComponent: {
      count: hardwareCount,
      findMany: hardwareFindMany,
      groupBy: hardwareGroupBy,
    },
...
describe('node data interface', () => {
  it('authorizes host resolution and returns safe purpose hosts', async () => {
    await expect(query.resolveNodeHost('h1')).resolves.toEqual({
      kind: 'selected',
      host: hosts[0],
      hosts,
    })
  ...
  it('tags every persistent read for eight hours', async () => {
    await query.getNodeOverview('h1')
    await query.getNodeTrend('h1')
    await query.getNodeResults(base)
    expect(cacheCalls.every((call) => call.options.revalidate === 28_800)).toBe(true)
    expect(
      cacheCalls.every((call) => call.options.tags.join('|') === 'nodes:all|nodes:host:h1'),
    ).toBe(true)
  })

  it('serializes overview and trend values inside the cache producer', async () => {
    expect(await query.getNodeOverview('h1')).toEqual({
      lastNodeSyncAt: '2026-01-01T00:00:00.000Z',
      nodesOnline: 2,
      nodesTotal: 3,
      componentsFailed: 1,
    })
    expect(await query.getNodeTrend('h1')).toEqual([
      { sampledAt: '2026-01-02T00:00:00.000Z', nodesOnline: 2, componentsFailed: 1 },
    ])
  })
