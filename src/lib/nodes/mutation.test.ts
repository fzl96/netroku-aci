...
describe('node mutation boundary', () => {
  it('shares sync semantics and immediately invalidates exact tags', async () => {
    ...
    await expect(mutation.resyncNodeInventory({ apicHostId: 'h1', username: ' admin ', password: 'pw' }))
      .resolves.toEqual({ ok: true, syncedNodes: 3, syncedComponents: 5, nodesOnline: 3 })
    expect(deps.resyncNodes).toHaveBeenCalledWith({ apicHostId: 'h1', host: 'apic.local', username: 'admin', password: 'pw' })
  ...
  it('does not let a failed audit skip invalidation', async () => {
    ...
    await expect(createNodeMutation(deps).resyncNodeInventoryForScheduler({
      apicHostId: 'h1', hostName: 'Fabric', host: 'apic.local', username: 'u', password: 'p',
  ...
  it('audits scheduler failures once and rethrows them without invalidation', async () => {
    const deps = dependencies()
    deps.resyncNodes.mockImplementation(async () => { throw new Error('offline') })
    await expect(createNodeMutation(deps).resyncNodeInventoryForScheduler({
      apicHostId: 'h1', hostName: 'Fabric', host: 'apic.local', username: 'u', password: 'p',
  ...
  it('maps only missing sessions and propagates auth infrastructure failures', async () => {
    ...
    missing.requireSession.mockImplementation(async () => { throw new AuthenticationRequiredError() })
    await expect(createNodeMutation(missing).resyncNodeInventory({ apicHostId: 'h1', username: 'u', password: 'p' }))
    ...
    broken.requireSession.mockImplementation(async () => { throw new Error('session database unavailable') })
    await expect(createNodeMutation(broken).resyncNodeInventory({ apicHostId: 'h1', username: 'u', password: 'p' }))
