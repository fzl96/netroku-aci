...
const NODE_SELECT = {
  id: true, nodeId: true, name: true, role: true, model: true, version: true,
  fabricSt: true, state: true, uptime: true,
} satisfies Prisma.NodeSnapshotSelect
const COMPONENT_SELECT = {
  id: true, nodeId: true, type: true, name: true, operSt: true, healthy: true, model: true,
...
export type NodeRow = {
  ...
  psu: NodeComponentCount; fan: NodeComponentCount
}
export type HardwareComponentRow = {
  id: string; nodeId: string; type: string; name: string; operSt: string; healthy: boolean; model: string
}
export type NodeOverviewData = {
  lastNodeSyncAt: string | null; nodesOnline: number; nodesTotal: number; componentsFailed: number
}
export type NodeTrendPoint = { sampledAt: string; nodesOnline: number; componentsFailed: number }
export type NodePagination = { page: number; pageSize: NodePageSize; total: number; totalPages: number }
...
export async function getNodeOverview(hostId: string): Promise<NodeOverviewData> {
  ...
  return unstable_cache(async () => {
    const [host, nodesTotal, nodesOnline, componentsFailed] = await Promise.all([
      ...
      prisma.nodeSnapshot.count({ where: {
        apicHostId: hostId, present: true,
        OR: [{ fabricSt: 'active' }, { role: 'controller', state: 'in-service' }],
      } }),
      prisma.hardwareComponent.count({ where: { apicHostId: hostId, present: true, healthy: false } }),
    ])
    return {
...
function serializeNode(row: StoredNode, counts: ComponentCount[]): NodeRow {
  return {
    id: row.id, nodeId: row.nodeId, name: row.name, role: row.role, model: row.model,
    version: row.version, fabricSt: row.fabricSt, state: row.state, uptime: row.uptime,
    psu: countsFor(counts, row.nodeId, 'psu'), fan: countsFor(counts, row.nodeId, 'fan'),
  }
}

function serializeComponent(row: StoredComponent): HardwareComponentRow {
  return {
    id: row.id, nodeId: row.nodeId, type: row.type, name: row.name,
    operSt: row.operSt, healthy: row.healthy, model: row.model,
  }
}
  ...
  > => {
    if (params.view === 'components') {
      const where: Prisma.HardwareComponentWhereInput = {
        apicHostId: params.hostId, present: true,
    ...
    const counts = records.length ? await prisma.hardwareComponent.groupBy({
      by: ['nodeId', 'type', 'healthy'],
      where: { apicHostId: params.hostId, present: true, nodeId: { in: records.map(row => row.nodeId) } },
      _count: { _all: true },
    }) : []
