...
<Callout type="warn" title="Windows users">
  Bun has a known compatibility issue with the TLS bypass used for self-signed APIC certificates.
  Use **WSL2** or Node.js instead.
</Callout>
...
<Callout type="info" title="Network access">
  The machine running the app must be able to reach your database host on the PostgreSQL port
  (default 5432). The database itself must already exist — migrations will create all tables.
</Callout>
